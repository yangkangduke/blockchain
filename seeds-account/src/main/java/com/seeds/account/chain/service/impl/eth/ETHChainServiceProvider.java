...
//@Slf4j
//@Data
//public class ETHChainServiceProvider extends ChainBasicService implements ChainService {
//
...
//
//    @Autowired
//    ChainProviderService chainProviderService;
//
...
//        ChainContractDto currencyConfig = chainContractService.get(chain.getCode(), currency);
//        Assert.isTrue(currencyConfig != null, "invalid currency " + currency);
//
//        BigInteger scaledAmount = scaleAmountByDecimal(amount, currencyConfig.getDecimals());
//        String routerAddress = systemWalletAddressService.getOne(chain, WalletAddressType.SWAP_ROUTER);
//        String data = chainFunctionConfig.getErc20ApproveFunctionEncoder().encodeWithFunctionParams(
//                Arrays.asList(new Address(routerAddress), new Uint256(scaledAmount)));
//
//        log.info("chainApprove - chain={} routerAddress={} currency={} fromAddress={} ammount={}",
//                chain, routerAddress, currency, address, amount);
//
//        BigInteger pendingNonce = getPendingNonce(chain, address);
...
//        rawTransactionDto.setGasPrice(BigInteger.valueOf(gasPrice));
//        rawTransactionDto.setGasLimit(BigInteger.valueOf(gasLimit));
//        rawTransactionDto.setTo(currencyConfig.getAddress());
//        rawTransactionDto.setData(data);
//        rawTransactionDto.setValue(BigInteger.ZERO);
//        rawTransactionSignRequest.setRawTransaction(rawTransactionDto);
//        rawTransactionSignRequest.setFromAddress(address);
...
//        if (ethSendTransaction.hasError()) {
//            log.error("chainApprove chain={} address={} currency={} amount={} err={}", chain, address, currency, amount, JsonUtils.writeValue(ethSendTransaction));
//            throw new ChainException(String.format("chainApprove err= %s", JsonUtils.writeValue(ethSendTransaction)));
//        }
//        String txHash = ethSendTransaction.getTransactionHash();
//        log.info("chainApprove chain={} address={} currency={} amount={} txHash={}", chain, address, currency, amount, txHash);
//        rawTransactionDto.setTxnHash(txHash);
//
//        return rawTransactionDto;
//    }
...
//        ChainContractDto currencyConfig = chainContractService.get(chain.getCode(), currency);
//        Assert.isTrue(currencyConfig != null, "invalid currency " + currency);
//        Assert.isTrue(address != null, "invalid address");
//
//        DynamicFunctionEncoder encoder = chainFunctionConfig.getErc20AllowanceFunctionEncoder();
//        String routerAddress = systemWalletAddressService.getOne(chain, WalletAddressType.SWAP_ROUTER);
//        String data = encoder.encodeWithFunctionParams(Arrays.asList(new Address(address), new Address(routerAddress)));
//
//        log.info("queryChainAllowance - routerAddress={} currency={} fromAddress={} address={}",
//                routerAddress, currency, currencyConfig.getAddress(), address);
//
//        Transaction transaction = Transaction.createEthCallTransaction(null, currencyConfig.getAddress(), data);
//        EthCall ethCall = web3.readCli().ethCall(transaction, DefaultBlockParameterName.LATEST).send();
//        if (ethCall.hasError()) {
//            log.error("queryChainAllowance error, err= {}", JsonUtils.writeValue(ethCall));
//            throw new ChainException(String.format("queryChainAllowance error, err= %s", JsonUtils.writeValue(ethCall)));
//        }
//        String result = ethCall.getValue();
//        List<Type> types = FunctionReturnDecoder.decode(result, encoder.getOutputParameters());
//        if (types != null && types.size() > 0) {
//            Uint256 res = (Uint256) (types.get(0));
//            Assert.isTrue(res != null, "queryChainAllowance ethCall got result size less than 2");
//            BigInteger value = res.getValue();
//            int decimals = currencyConfig.getDecimals();
//            BigDecimal allowance = unscaleAmountByDecimal(value, decimals);
//            log.info("queryChainAllowance - allowance={}", allowance);
//            return allowance;
//        }
//        throw new ChainException(String.format("queryChainAllowance error, result= %s", result));
//    }
//
//    @Override
...
//        Assert.isTrue(mcdPrice.compareTo(BigDecimal.ZERO) > 0, "MCD price should not be 0");
//
//        // If deltaKusd is negative, check kUSD vault contract balance is enough or not
//        if (deltaKusd.signum() < 0) {
//            String kusdVaultAddress = systemWalletAddressService.getOne(chain, WalletAddressType.KUSD_VAULT);
//            BigDecimal vaultKusdBalance = getContractBalance(chain, kusdVaultAddress, AccountConstants.KUSD_CURRENCY);
//            if (vaultKusdBalance.compareTo(deltaKusd.negate()) < 0) {
//                throw new AddressBalanceNotSufficientException(String.format("kUSD vault contract balance not sufficient to burn %s, need to transfer %s",
//                        deltaKusd.negate().toPlainString(), deltaKusd.negate().subtract(vaultKusdBalance).toPlainString()));
//            }
//        }
//        // current KINE price, get from CEX
//        BigDecimal currentKinePrice = priceService.getLatestPrice(AccountConstants.KINE.concat(AccountConstants.USD));
//        BigDecimal currentWooPrice = priceService.getLatestPrice(AccountConstants.WOO.concat(AccountConstants.USD));
//        // Get the current owned prices
//        ChainOraclePricesDto chainOraclePricesDto = getOraclePrices(chain, Stream.of(AccountConstants.MCD, AccountConstants.KINE, AccountConstants.WOO).collect(Collectors.toSet()));
//        BigDecimal chainMcdPrice = chainOraclePricesDto.getPrices().get(AccountConstants.MCD);
//        BigDecimal chainKinePrice = chainOraclePricesDto.getPrices().get(AccountConstants.KINE);
//        BigDecimal chainWooPrice = chainOraclePricesDto.getPrices().get(AccountConstants.WOO);
//
//        // 将链上报时间统一到分钟精度级别，防止多链上报拿到的timestamp不同，导致check上报时间，有的链需要上报，有的不需要
//        // e.g. ETH 上次上报时间为 00:00:10, BSC 为 00:00:12, 本次上报 ETH 00:10:09, BSC 00:10:13, 上报间隔配置为10min
//        // 则 ETH 不需要上报（00:00:10 - 00:10:09 < 10min）, BSC 需要上报。 这会造成系统对ETH上报失败，BSC正常上报，ETH上报触发retry机制
//        // 其实ETH实际是正常上报，不走retry
//        Long chainMcdTs = chainOraclePricesDto.getTimestamp() / 60 * 60;
//        log.info("reportKaptain, chain={}, chainMcdPrice={}, chainMcdTs={}, lastTimestamp={}, timestamp={}", chain, chainMcdPrice, chainMcdTs, lastTimestamp, timestamp);
//        BigDecimal mcdReporterVolatility = new BigDecimal(systemConfigService.getValue(AccountSystemConfig.CHAIN_REPORTER_VOLATILITY));
//        BigDecimal kineReporterVolatility = new BigDecimal(systemConfigService.getValue(AccountSystemConfig.CHAIN_KINE_VOLATILITY));
//        BigDecimal wooReporterVolatility = new BigDecimal(systemConfigService.getValue(AccountSystemConfig.CHAIN_WOO_VOLATILITY));
//
//        Long mcdReporterPeriod = Long.parseLong(systemConfigService.getValue(AccountSystemConfig.CHAIN_REPORTER_PERIOD, "30")) * 60;
//        Long kineReporterPeriod = Long.parseLong(systemConfigService.getValue(AccountSystemConfig.CHAIN_KINE_PERIOD, "30")) * 60;
//        Long wooReporterPeriod = Long.parseLong(systemConfigService.getValue(AccountSystemConfig.CHAIN_WOO_PERIOD, "30")) * 60;
//        BigDecimal ceilingVolatility = new BigDecimal(systemConfigService.getValue(AccountSystemConfig.CHAIN_REPORTER_CEILING_VOLATILITY));
//        boolean mcdVolatilityNotExceed = chainMcdPrice.signum() > 0 ? mcdPrice.subtract(chainMcdPrice)
//                .divide(chainMcdPrice, DEFAULT_SCALE, RoundingMode.HALF_EVEN)
//                .abs().compareTo(ceilingVolatility) < 0 : true;
//        log.info("reportKaptain, chain={}, chainMcdTs={}, timestamp={}, " +
//                        "chainMcdPrice={}, mcdPrice={}, " +
//                        "chainKinePrice={}, currentKinePrice={}, " +
//                        "chainWooPrice={}, wooCexPrice={}",
//                chain, chainMcdTs, timestamp, chainMcdPrice, mcdPrice, chainKinePrice, currentKinePrice, chainWooPrice, currentWooPrice);
//        // MCD波动率超过上限&&超过skip次数, Price change > 1%, or the time elapsed more than 30 mins, do post
//        boolean isReportMcd = checkVolatilityAndTimeElapsed(chainMcdTs, lastTimestamp, timestamp, chainMcdPrice, mcdPrice, mcdReporterVolatility, mcdReporterPeriod);
//        // Price change > 1%, or the time elapsed more than 30 mins, do post
//        boolean isReportKine = checkVolatilityAndTimeElapsed(chainMcdTs, lastTimestamp, timestamp, chainKinePrice, currentKinePrice, kineReporterVolatility, kineReporterPeriod);
//        boolean isReportWoo = checkVolatilityAndTimeElapsed(chainMcdTs, lastTimestamp, timestamp, chainWooPrice, currentWooPrice, wooReporterVolatility, wooReporterPeriod);
//
//        log.info("reportKaptain, chain={}, mcdReporterVolatility={}, kineReporterVolatility={}, wooReporterVolatility={}, " +
//                        "mcdReporterPeriod={}, kineReporterPeriod={}, wooReporterPeriod={}, " +
//                        "mcdVolatilityNotExceed={}, " +
//                        "isReportMcd={}, isReportKine={}, isReportWoo={}",
//                chain, mcdReporterVolatility, kineReporterVolatility, wooReporterVolatility,
//                mcdReporterPeriod, kineReporterPeriod, wooReporterPeriod,
//                mcdVolatilityNotExceed,
//                isReportMcd, isReportKine, isReportWoo);
//        if (isRetry ||
//                chainMcdPrice.signum() == 0 ||
//                chainKinePrice.signum() == 0 ||
//                (mcdVolatilityNotExceed && (isReportMcd || isReportKine || isReportWoo)) ||
//                isCeilingReport(chainMcdPrice, mcdPrice)) {
//            boolean isVaultIncrease = deltaKusd.compareTo(BigDecimal.ZERO) >= 0;
//            List<Utf8String> callSymbols = Arrays.asList(new Utf8String(AccountConstants.KINE), new Utf8String(AccountConstants.XKINE), new Utf8String(AccountConstants.WOO));
//            List<Uint> prices = Arrays.asList(
//                    new Uint(new BigInteger(currentKinePrice.multiply(
//                            new BigDecimal(getKTokenConfigBySymbol(chain, AccountConstants.KINE).getPriceUnit().getValue()))
//                            .setScale(0, RoundingMode.DOWN).toPlainString())),
//                    new Uint(new BigInteger(currentKinePrice.multiply(
//                            new BigDecimal(getKTokenConfigBySymbol(chain, AccountConstants.XKINE).getPriceUnit().getValue()))
//                            .setScale(0, RoundingMode.DOWN).toPlainString())),
//                    new Uint(new BigInteger(currentWooPrice.multiply(
//                            new BigDecimal(getKTokenConfigBySymbol(chain, AccountConstants.WOO).getPriceUnit().getValue()))
//                            .setScale(0, RoundingMode.DOWN).toPlainString()))
//            );
//
//            DynamicArray<Utf8String> symbolArray = new DynamicArray<>(Utf8String.class, callSymbols);
//            DynamicArray<Uint> pricesArray = new DynamicArray<>(Uint.class, prices);
//            String kaptainAddress = systemWalletAddressService.getOne(chain, WalletAddressType.KAPTAIN);
//
//            // Get current kusdDelta nonce of Kaptain
//            DynamicFunctionEncoder kaptainReporterNonceFunction = chainFunctionConfig.getKaptainNonceFunctionEncoder();
//            List<Type> reporterNonceReturn =
//                    viewOnlyChainFunctionCall(kaptainReporterNonceFunction, Collections.emptyList(), kaptainAddress);
//            Uint256 kaptainMcdReportNonceChainResult = (Uint256) reporterNonceReturn.get(0);
//            BigInteger kaptainMcdReportNonce = kaptainMcdReportNonceChainResult.getValue();
//            log.info("reportKaptain, chain={} - fetched chain reporterNonce is: {}", chain, kaptainMcdReportNonce.intValue());
//            if (!kaptainMcdReportNonce.add(BigInteger.ONE).equals(nonce)) {
//                throw new ChainNonceException(String.format("chain=%s, invalid nonce, expected next nonce=%s, chain current nonce=%s",
//                        chain, nonce, kaptainMcdReportNonce));
//            }
//
//            // bytes[] memory messages, bytes[] memory signatures, string[] memory symbols, uint256 vaultKusdDelta, bool isVaultIncreased, uint256 nonce
//            String steerMsgToSign =
//                    encodeMessageByParams(chain,
//                            Arrays.asList(
//                                    symbolArray,
//                                    pricesArray,
//                                    new Uint256(scaleAmountByDecimal(deltaKusd, 18)),
//                                    new Bool(isVaultIncrease),
//                                    new Uint256(kaptainMcdReportNonce.add(BigInteger.ONE))));
//
//            SignMessageRequest signSteerMsgRequest =
//                    SignMessageRequest.builder()
//                            .messagesToSign(Collections.singletonList(steerMsgToSign))
//                            .address(reporterAddress)
//                            .build();
//            List<SignedMessageDto> signedSteerMsgList = signMessages(signSteerMsgRequest).getData();
//
//            String signedSteerMsg = signedSteerMsgList.get(0).getMessage();
//            String signedSteerSig = signedSteerMsgList.get(0).getSignature();
//
//            DynamicFunctionEncoder steerFunction = chainFunctionConfig.getKaptainSteerFunctionEncoder();
//            List<Type> steerParams =
//                    Arrays.asList(
//                            new DynamicBytes(Numeric.hexStringToByteArray(signedSteerMsg)),
//                            new DynamicBytes(Numeric.hexStringToByteArray(signedSteerSig)));
//
//            return stateChangeChainFunctionCall(chain, steerFunction, steerParams, kaptainAddress, posterAddress);
//        }
//
//        return null;
//    }
...
//        DynamicFunctionEncoder priceScale36Encoder = chainFunctionConfig.getPriceScale36Encoder();
//        String oracleAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE);
//        String oracleHelperAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE_HELPER);
//
//        long timestamp = 0;
//        Map<String, BigDecimal> priceMap = new HashMap<>();
//        for (String tmpSymbol : currency) {
//            String tmpData = priceScale36Encoder.encodeWithFunctionParams(
//                    Arrays.asList(new Address(oracleAddress), new Utf8String(tmpSymbol)));
//            Transaction tmpTransaction =
//                    Transaction.createEthCallTransaction(null, oracleHelperAddress, tmpData);
//            String tmpResult = web3.readCli().ethCall(tmpTransaction, DefaultBlockParameterName.LATEST).send().getValue();
//            List<Type> tmpReturns =
//                    FunctionReturnDecoder.decode(tmpResult, priceScale36Encoder.getOutputParameters());
//            ChainOracleHelperPriceDto tmpPrice = ChainOracleHelperPriceDto.createInstance(tmpReturns);
//            long tmpTimestamp = tmpPrice.getTimestamp().getValue().longValue();
//            if (timestamp == 0) {
//                timestamp = tmpTimestamp;
//            } else {
//                Assert.isTrue(timestamp == tmpPrice.getTimestamp().getValue().longValue(),
//                        "Price timestamp does not match");
//            }
//            priceMap.put(tmpPrice.getSymbol().toString(), unscaleAmountByDecimal(tmpPrice.getPrice().getValue(), 36));
//        }
//
//        return ChainOraclePricesDto.builder().prices(priceMap).timestamp(timestamp).build();
//    }
//
//    @Override
//    public ChainKTokenConfigDto getKTokenConfigBySymbol(Chain chain, String currency) throws IOException {
//        DynamicFunctionEncoder encoder = chainFunctionConfig.getKTokenConfigEncoder();
//        String oracleAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE);
//        String oracleHelperAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE_HELPER);
//        String tmpData = encoder.encodeWithFunctionParams(
//                Arrays.asList(new Address(oracleAddress), new Utf8String(currency)));
//        Transaction tmpTransaction =
//                Transaction.createEthCallTransaction(null, oracleHelperAddress, tmpData);
//        String tmpResult = web3.readCli().ethCall(tmpTransaction, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> tmpReturns =
//                FunctionReturnDecoder.decode(tmpResult, encoder.getOutputParameters());
//        return ChainKTokenConfigDto.createInstance(tmpReturns);
//    }
//
//    @Override
//    public ChainAnchorPriceView getAnchorPriceView(Chain chain, String currency, String oracleHelperAddress) throws IOException {
//        DynamicFunctionEncoder anchorPriceViewEncoder = chainFunctionConfig.getAnchorPriceViewFunctionEncoder();
//        String oracleAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE);
//        String currencyData =
//                anchorPriceViewEncoder.encodeWithFunctionParams(
//                        Arrays.asList(new Address(oracleAddress), new Utf8String(currency)));
//        Transaction currencyTransaction =
//                Transaction.createEthCallTransaction(null, oracleHelperAddress, currencyData);
//        String kineResult = web3.readCli().ethCall(currencyTransaction, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> currencyReturn =
//                FunctionReturnDecoder.decode(kineResult, anchorPriceViewEncoder.getOutputParameters());
//        return ChainAnchorPriceView.createInstance(currencyReturn);
//    }
//
//    @Override
...
//        DynamicFunctionEncoder encoder = chainFunctionConfig.getOwnableTrasferOwnerFunctionEncoder();
//        String data = encoder.encodeWithFunctionParams(Arrays.asList(new Address(newOwnerAddress)));
//
//        BigInteger pendingNonce = getPendingNonce(chain, address);
//
//        RawTransactionSignRequest rawTransactionSignRequest = new RawTransactionSignRequest();
...
//        rawTransactionDto.setGasPrice(BigInteger.valueOf(getGasPrice(chain)));
//        rawTransactionDto.setGasLimit(BigInteger.valueOf(getGasLimit(chain)));
//        rawTransactionDto.setTo(ownableContractAddress);
//        rawTransactionDto.setData(data);
//        rawTransactionDto.setValue(BigInteger.ZERO);
//
//        rawTransactionSignRequest.setRawTransaction(rawTransactionDto);
...
//        EthSendTransaction ethSendTransaction = web3.writeCli().ethSendRawTransaction(signTransaction).send();
//        if (ethSendTransaction.hasError()) {
//            log.error("transferOwnership err= {}", JsonUtils.writeValue(ethSendTransaction));
//            throw new ChainException(String.format("transferOwnership err= %s", JsonUtils.writeValue(ethSendTransaction)));
//        }
//        String txHash = ethSendTransaction.getTransactionHash();
//        log.info("transferOwnership got txHash: {}", txHash);
//        rawTransactionDto.setTxnHash(txHash);
//
//        return rawTransactionDto;
//    }
...
//        String mcdAddress = chainContractService.get(chain.getCode(), AccountConstants.MCD).getAddress();
//        String kineUnitrollerAddress = systemWalletAddressService.getOne(chain, WalletAddressType.KINE_UNITROLLER);
//
//        DynamicFunctionEncoder functionEncoder = chainFunctionConfig.getAllMarketsFunctionEncoder();
//        String callData = functionEncoder.encodeWithFunctionParams(Collections.emptyList());
//        Transaction txn = Transaction.createEthCallTransaction(null, kineUnitrollerAddress, callData);
//        String res = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> returns = FunctionReturnDecoder.decode(res, functionEncoder.getOutputParameters());
//        Address kmcdAddress = new Address(mcdAddress);
//
//        // returned markets array
//        DynamicArray<Address> returnedAddressArray = (DynamicArray<Address>) returns.get(0);
//
//        Set<Address> ignoreMarkets = getIgnoreMarkets(chain);
//
//        List<Address> marketStrings = returnedAddressArray.getValue().stream()
//                .filter(m -> !ignoreMarkets.contains(m))
//                .filter(m -> !m.equals(kmcdAddress)).collect(Collectors.toList());
//        DynamicFunctionEncoder helperEncoder = chainFunctionConfig.getKineProtocolHelperFunctionEncoder();
//        String oracleAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE);
//        String kineProtocolHelperAddress = systemWalletAddressService.getOne(chain, WalletAddressType.KINE_PROTOCOL_HELPER);
//        String helperCallData = helperEncoder.encodeWithFunctionParams(Arrays.asList(new DynamicArray<>(Address.class, marketStrings), new Address(oracleAddress)));
//        Transaction helperTxn = Transaction.createEthCallTransaction(null, kineProtocolHelperAddress, helperCallData);
//        String helperRes = web3.readCli().ethCall(helperTxn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> helperReturn = FunctionReturnDecoder.decode(helperRes, helperEncoder.getOutputParameters());
//        Uint helperReturnInt = (Uint) helperReturn.get(0);
//        return unscaleAmountByDecimal(helperReturnInt.getValue(), 36);
//    }
//
//    @Override
...
//        List<String> addressList = addressAssetMap.keySet().stream().collect(Collectors.toList());
//
//        String oracleAddress = systemWalletAddressService.getOne(chain, WalletAddressType.ORACLE);
//        String kineProtocolHelperAddress = systemWalletAddressService.getOne(chain, WalletAddressType.KINE_PROTOCOL_HELPER);
//
//        DynamicFunctionEncoder helperEncoder = chainFunctionConfig.getKineProtocolHelperStakeValueFunctionEncoder();
//
//        Map<String, BigDecimal> assetStakingValueMap = new HashMap<>();
//        for (String address : addressList) {
//            String helperCallData = helperEncoder.encodeWithFunctionParams(Arrays.asList(new Address(address), new Address(oracleAddress)));
//            Transaction helperTxn = Transaction.createEthCallTransaction(null, kineProtocolHelperAddress, helperCallData);
//            String helperRes = web3.readCli().ethCall(helperTxn, DefaultBlockParameterName.LATEST).send().getValue();
//            List<Type> helperReturn = FunctionReturnDecoder.decode(helperRes, helperEncoder.getOutputParameters());
//            Uint helperReturnInt = (Uint) helperReturn.get(0);
//            assetStakingValueMap.put(addressAssetMap.get(address).getCurrency(), unscaleAmountByDecimal(helperReturnInt.getValue(), addressAssetMap.get(address).getDecimals()));
//        }
//
//        return assetStakingValueMap;
//    }
//
//    @Override
//    public BigDecimal getTotalSupplyByAddressAndDecimals(Chain chain, String address, Integer decimals) throws IOException {
//        String callData =
//                chainFunctionConfig.getErc20TotalSupplyFunctionEncoder().encodeWithFunctionParams(Collections.emptyList());
//        Transaction txn =
//                Transaction.createEthCallTransaction(null, address, callData);
//        String res = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> returns =
//                FunctionReturnDecoder.decode(res, chainFunctionConfig.getErc20TotalSupplyFunctionEncoder().getOutputParameters());
//        Uint totalSupplyUint = returns.isEmpty() ? new Uint(BigInteger.ZERO) : (Uint) returns.get(0);
//        return unscaleAmountByDecimal(totalSupplyUint.getValue(), decimals);
//    }
//
//    @Override
...
//        String callData =
//                chainFunctionConfig.getErc20BalanceOfFunctionEncoder()
//                        .encodeWithFunctionParams(Arrays.asList(new Address(accountAddress)));
//        Transaction txn =
//                Transaction.createEthCallTransaction(null, tokenAddress, callData);
//        String res = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> returns =
//                FunctionReturnDecoder.decode(res, chainFunctionConfig.getErc20BalanceOfFunctionEncoder().getOutputParameters());
//        Uint balance = returns.isEmpty() ? new Uint(BigInteger.ZERO) : (Uint) returns.get(0);
//        return unscaleAmountByDecimal(balance.getValue(), tokenDecimal);
//    }
//
//    @Slf4j
...
//    }
//
//    private RawTransactionDto stateChangeChainFunctionCall(Chain chain, DynamicFunctionEncoder functionEncoder,
//                                                           List<Type> params,
//                                                           String toAddress,
//                                                           String fromAddress) throws IOException {
//        String callData =
//                functionEncoder.encodeWithFunctionParams(params);
//
//        BigInteger pendingNonce = getPendingNonce(chain, fromAddress);
//        long gasPrice = getGasPrice(chain);
//        long gasLimit = getGasLimit(chain);
//        RawTransactionSignRequest rawTransactionSignRequest = new RawTransactionSignRequest();
//        RawTransactionDto rawTransactionDto = new RawTransactionDto();
...
//        rawTransactionDto.setGasLimit(BigInteger.valueOf(gasLimit));
//        rawTransactionDto.setTo(toAddress);
//        rawTransactionDto.setData(callData);
//        rawTransactionSignRequest.setRawTransaction(rawTransactionDto);
//        rawTransactionSignRequest.setFromAddress(fromAddress);
//        String signTransaction = sign(rawTransactionSignRequest).getData();
//        EthSendTransaction ethSendTransaction = web3.writeCli().ethSendRawTransaction(signTransaction).send();
//        if (ethSendTransaction.hasError()) {
//            log.error("error when send reportKaptain transaction, chain={}, err= {}", chain, JsonUtils.writeValue(ethSendTransaction));
//            throw new ChainException(String.format("error when send reportKaptain transaction, chain=%s, err= %s", chain, JsonUtils.writeValue(ethSendTransaction)));
//        }
//        String txHash = ethSendTransaction.getTransactionHash();
//        log.info("Kaptain chain={}, steer txHash={}", chain, txHash);
//        rawTransactionDto.setTxnHash(txHash);
//        return rawTransactionDto;
//    }
//
//    private List<Type> viewOnlyChainFunctionCall(DynamicFunctionEncoder functionEncoder,
//                                                 List<Type> params,
//                                                 String toAddress) throws IOException {
//        String callData =
//                functionEncoder.encodeWithFunctionParams(params);
//        Transaction txn =
//                Transaction.createEthCallTransaction(null, toAddress, callData);
//
//        String viewCallResult = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//
//        return FunctionReturnDecoder.decode(viewCallResult, functionEncoder.getOutputParameters());
//    }
//
...
//    }
//
//    private boolean isCeilingReport(BigDecimal previousPrice, BigDecimal currentPrice) {
//        BigDecimal ceilingVolatility = new BigDecimal(systemConfigService.getValue(AccountSystemConfig.CHAIN_REPORTER_CEILING_VOLATILITY));
//        int ceilingTimes = Integer.parseInt(systemConfigService.getValue(AccountSystemConfig.CHAIN_REPORTER_CEILING_TIMES));
//
//        boolean mcdVolatilityExceed = currentPrice.subtract(previousPrice)
//                .divide(previousPrice, DEFAULT_SCALE, RoundingMode.HALF_EVEN)
//                .abs().compareTo(ceilingVolatility) >= 0;
//
//        RBucket<String> rBucket = client.getBucket(RedisKeys.getChainMcdSkipTimes(), StringCodec.INSTANCE);
//        int skipTimes = rBucket.isExists() && !StringUtils.isEmpty(rBucket.get()) ? Integer.parseInt(rBucket.get()) : 0;
//        if (mcdVolatilityExceed) {
//            // 超过浮动上限
...
//                            "module", "mcd",
//                            "content", String.format("mcd price exceeded ceiling volatility %s times, setting times is %s", skipTimes, ceilingTimes)))
//                    .build());
//            log.warn("isCeilingReport, mcd price exceeded ceiling volatility {} times, setting times is {}", skipTimes, ceilingTimes);
//            if (skipTimes > ceilingTimes) {
//                // 超过
//                rBucket.set("0");
//                return true;
//            }
//            rBucket.set(String.valueOf(skipTimes));
//            return false;
//        }
//        rBucket.set("0");
//        return false;
...
//            MetricsGaugeUtils.gauge("chainGasPrice", Tags.of("type", "safeGasPrice", "chain", chain.getName()), Double.parseDouble(response.getResult().getSafeGasPrice()));
//            MetricsGaugeUtils.gauge("chainGasPrice", Tags.of("type", "proposeGasPrice", "chain", chain.getName()), Double.parseDouble(response.getResult().getProposeGasPrice()));
//            MetricsGaugeUtils.gauge("chainGasPrice", Tags.of("type", "fastGasPrice", "chain", chain.getName()), Double.parseDouble(response.getResult().getFastGasPrice()));
...
//            List<BatchBalanceResponseProcessor> batchBalanceResponseProcessors = Lists.newArrayList();
//
...
//                BatchBalanceResponseProcessor batchBalanceResponseProcessor = batchBalanceResponseProcessors.get(i);
...
//    }
//
//    public void getNativeTokenBalanceOnBatch(BatchRequest batchRequest, List<BatchBalanceResponseProcessor> batchBalanceResponseProcessors, String address) {
...
//    }
//
//    private void getContractBalanceOnBatch(BatchRequest batchRequest, List<BatchBalanceResponseProcessor> batchBalanceResponseProcessors,
...
//        DynamicFunctionEncoder encoder = new DynamicFunctionEncoder("claimHistory(uint256)", Arrays.asList(new TypeReference<Uint256>() {
//        }));
//        String input = encoder.encodeWithFunctionParams(Arrays.asList(new Uint256(id)));
//
//        Transaction txn = Transaction.createEthCallTransaction(null, contractAddress, input);
//
//        String output = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> returns = FunctionReturnDecoder.decode(output, encoder.getOutputParameters());
//        return returns != null && returns.size() > 0 ? ((Uint256) returns.get(0)).getValue().longValue() : 0;
//    }
//
//    @Override
//    public BigDecimal getTotalStakes(Chain chain, String address, int decimals) throws Exception {
//        String callData =
//                chainFunctionConfig.getErc20TotalStakesFunctionEncoder().encodeWithFunctionParams(Collections.emptyList());
//        Transaction txn =
//                Transaction.createEthCallTransaction(null, address, callData);
//        String res = web3.readCli().ethCall(txn, DefaultBlockParameterName.LATEST).send().getValue();
//        List<Type> returns =
//                FunctionReturnDecoder.decode(res, chainFunctionConfig.getErc20TotalStakesFunctionEncoder().getOutputParameters());
//        Uint totalStakesUint = returns.isEmpty() ? new Uint(BigInteger.ZERO) : (Uint) returns.get(0);
//        return unscaleAmountByDecimal(totalStakesUint.getValue(), decimals);
//    }
//
//    @Override
