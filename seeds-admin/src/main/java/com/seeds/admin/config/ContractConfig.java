...
/**
    ...
    @Scope("prototype")
    @Autowired
    public EthFilter filter(GameItems gameItems, Web3j  web3j) throws IOException {
        //获取启动时监听的区块
        Request<?, EthBlockNumber> request = web3j.ethBlockNumber();
        BigInteger block = request.send().getBlockNumber();
        log.info("block number is:{}-{}", block, DefaultBlockParameterName.LATEST);
        return new EthFilter(DefaultBlockParameter.valueOf(block),
                DefaultBlockParameterName.LATEST, gameItems.getContractAddress());
    }
